...
impl TryFrom<Ticket> for receipts::Receipt {
    ...
    fn try_from(value: Ticket) -> Result<Self, Self::Error> {
        match value.details {
            TicketDetails::NATIVE(ticket_native) => {
                let issued_at = DateTime::parse_from_rfc3339(&value.summary.date)
                    .or_else(|_| DateTime::parse_from_rfc3339(&ticket_native.date))
                    .map(|d| d.to_utc())
                    .or_else(|_| {
                        ...
                        .map(|d| {
                            match d.and_local_timezone(Local).earliest() {
                                Some(d) => d.to_utc(),
                                None => d.and_utc(),
                            }
                        })
                    ...
                    .map_err(|e| {
                        anyhow!(
                            "Failed to parse date: {} (summary: {}, native: {})",
                            e,
                            value.summary.date,
                            ticket_native.date
                        )
                ...
                let items = ticket_native
                    .items_line
                    .into_iter()
                    .map(|i| -> Result<ReceiptItem, Self::Error> {
                        let current_unit_price =
                            i.current_unit_price.replace(",", ".").parse::<f32>()?;
                        let quantity = i.quantity.replace(",", ".").parse::<f32>()?;
                        let tax_rate = ticket_native
                            .taxes
                            .iter()
                            .find(|f| f.tax_group_name == i.tax_group_name)
                            .map(|t| -> Result<f32, Self::Error> {
                                Ok(t.percentage.replace(",", ".").parse::<f32>()? / 100.0)
                            })
                            .transpose()?;
                ...
                let tax_total_val = match &ticket_native.total_taxes {
                ...
                let payments = ticket_native
                    .payments
                    .into_iter()
                    .map(|p| -> Result<ReceiptPayment, Self::Error> {
                        Ok(ReceiptPayment::new(
            ...
            TicketDetails::HTML(ticket_html) => {
                let issued_at = DateTime::parse_from_rfc3339(&value.summary.date)
                    .or_else(|_| DateTime::parse_from_rfc3339(&ticket_html.date))
                    .map(|d| d.to_utc())
                    .or_else(|_| {
                        chrono::NaiveDateTime::parse_from_str(
                            &ticket_html.date,
                    ...
                    .map_err(|e| {
                        anyhow!(
                            "Failed to parse date: {} (summary: {}, html: {})",
                            e,
                            value.summary.date,
                            ticket_html.date
                        )
                ...
                let document = Html::parse_document(&ticket_html.html_printed_receipt);
                let purchase_list_selector = Selector::parse("span.purchase_list").unwrap();

                let mut items = Vec::new();

                if let Some(purchase_list) = document.select(&purchase_list_selector).next() {
                    let mut current_item: Option<ReceiptItem> = None;
                    let mut current_art_id: Option<String> = None;

                    for node in purchase_list.children() {
                        let element = match node.value().as_element() {
                            Some(e) => e,
                            None => continue,
                        };

                        if element.has_class("article", scraper::CaseSensitivity::CaseSensitive) {
                            let art_id = element.attr("data-art-id").map(|s| s.to_string());
                            let description = element
                                .attr("data-art-description")
                                .map(|s| s.to_string())
                                .unwrap_or_default();
                            let unit_price = element
                                .attr("data-unit-price")
                                .and_then(|s| s.replace(",", ".").parse::<f32>().ok())
                                .unwrap_or(0.0);
                            let quantity = element
                                .attr("data-art-quantity")
                                .and_then(|s| s.replace(",", ".").parse::<f32>().ok())
                                .unwrap_or(1.0);
                            let tax_type = element.attr("data-tax-type").map(|s| s.to_string());

                            if let Some(item) = current_item.take() {
                                if art_id.is_some() && art_id == current_art_id {
                                    // Merge spans for the same article
                                    let name = if item.get_name().is_empty() {
                                        description
                                    } else {
                                        item.get_name()
                                    };
                                    let price = if item.get_price() == 0.0 {
                                        unit_price
                                    } else {
                                        item.get_price()
                                    };
                                    let count = if item.get_count() <= 1.0 {
                                        quantity
                                    } else {
                                        item.get_count()
                                    };
                            ...
                            .has_class("discount", scraper::CaseSensitivity::CaseSensitive)
                        {
                            if let Some(mut item) = current_item.take() {
                                ...
                                if discount_val != 0.0 {
                                    let new_total = item.get_total() + discount_val;
                ...
                let vat_selector = Selector::parse("span.vat_info span").unwrap();
                let mut tax_summary = Vec::new();
                let mut tax_total_f = 0.0;

                for vat_line in document.select(&vat_selector) {
                    let tax_type = match vat_line.value().attr("data-tax-type") {
                        Some(t) => t,
                        None => continue,
                    };
                    let percentage = vat_line
                        .value()
                        .attr("data-tax-percentage")
                        .and_then(|s| s.replace(",", ".").parse::<f32>().ok())
                        .unwrap_or(0.0)
                        / 100.0;
                    let base_amount = vat_line
                        .value()
                        .attr("data-tax-base-amount")
                        .and_then(|s| s.replace(",", ".").parse::<f32>().ok())
                        .unwrap_or(0.0);
                    let tax_amount = vat_line
                        .value()
                        .attr("data-tax-amount")
                        .and_then(|s| s.replace(",", ".").parse::<f32>().ok())
                        .unwrap_or(0.0);

                    if !tax_summary
                        .iter()
                        .any(|ts: &ReceiptTaxSummary| ts.tax_group() == Some(tax_type.to_string()))
                    {
                ...
                let summary_selector = Selector::parse("span.purchase_summary span").unwrap();
                let mut payments = Vec::new();

                for summary_line in document.select(&summary_selector) {
                    if let Some(tender) = summary_line.value().attr("data-tender-description") {
                        let text = summary_line.text().collect::<String>();
                        let amount = text
                            .split_whitespace()
                            .last()
                            .and_then(|s| s.replace(",", ".").parse::<f32>().ok())
                        ...
                        payments.push(ReceiptPayment::new(
                            match tender {
                                "Gotówka" => receipts::ReceiptPaymentType::Cash,
...
pub struct TicketNative {
    ...
    pub total_taxes: Option<TicketNativeTotalTaxes>,
...
mod tests {
    ...
    use crate::api::{
        receipts::Receipt,
        retailers::lidl::models::{Ticket, TicketDetails, TicketsPage},
    };

    #[test]
    fn parse_html() -> Result<()> {
        let page_data = r#"{"page":1,"size":25,"totalCount":1,"tickets":[{"id":"240013888620260411403662","isFavorite":false,"date":"2026-04-11T11:24:10+00:00","currency":{"code":"PLN","symbol":"zł"},"totalAmount":39.45,"storeCode":"PL1388","articlesCount":5,"couponsUsedCount":3,"vendor":null,"isHtml":true,"hasHtmlDocument":true,"returns":[],"iconUrl":"https://static-tickets.lidlplus.com/images/assets/ticketdetail/ticket.png","badges":{"coupons":3,"invoice":false,"returns":0,"isAvailable":true}}]}"#;
        ...
        let ticket_result = serde_json::from_str::<TicketsPage>(page_data);
        let tickets_page = match ticket_result {
            Ok(receipt) => receipt,
            Err(e) => {
                error!("Failed to parse ticketPage JSON: {:?}", e);
                return Err(e.into());
            }
        };

        let ticket_details_result = serde_json::from_str::<TicketDetails>(test_data);
        trace!("Lidl::ticket parsed ticket details result");
        let ticket_details = match ticket_details_result {
            Ok(details) => details,
            Err(e) => {
                error!("Failed to parse TicketDetails JSON: {:?}", e);
                trace!(
        ...
        let ticket = Ticket::new(tickets_page.tickets.get(0).unwrap().clone(), ticket_details);

        dbg!(&ticket);

        let receipt = Receipt::try_from(ticket);
    ...
    #[test]
    fn test_native_ticket_to_receipt() -> Result<()> {
        let page_data = r#"{"page":1,"size":25,"totalCount":1,"tickets":[{"id":"24001388120230520279191","isFavorite":false,"date":"2023-05-20T09:04:28+00:00","currency":{"code":"PLN","symbol":"zł"},"totalAmount":102.54,"storeCode":"PL1388","articlesCount":13,"couponsUsedCount":0,"vendor":null,"isHtml":false,"hasHtmlDocument":false,"returns":[],"iconUrl":"https://static-tickets.lidlplus.com/images/assets/ticketdetail/ticket.png","badges":{"coupons":0,"invoice":false,"returns":0,"isAvailable":true}}]}"#;
